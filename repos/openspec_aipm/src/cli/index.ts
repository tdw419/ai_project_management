...
changeCmd
  ...
  .option('--slices <n>', 'Number of slices to create (default: 2)')
  .option('--names <names>', 'Comma-separated names for child slices')
  .option('--overwrite', 'Allow re-splitting an already-split change')
  .option('--force', 'Alias for --overwrite')
  .option('--json', 'Output as JSON')
  .action(async (changeId: string, options?: { slices?: string; names?: string; overwrite?: boolean; force?: boolean; json?: boolean }) => {
    try {
      const changeCommand = new ChangeCommand();
      await changeCommand.split(changeId, {
        slices: options?.slices ? parseInt(options.slices, 10) : undefined,
        names: options?.names ? options.names.split(',').map((s: string) => s.trim()) : undefined,
