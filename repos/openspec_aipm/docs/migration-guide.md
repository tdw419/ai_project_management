...
## Migrating from IMPLEMENTATION_ORDER.md

Some teams maintain an `openspec/changes/IMPLEMENTATION_ORDER.md` file as a manual narrative describing which changes should land first and why. With change stacking, machine-readable metadata in each change's `.openspec.yaml` now handles dependency ordering automatically.

### What Changes

| Aspect | Before (IMPLEMENTATION_ORDER.md) | After (stack metadata) |
|--------|----------------------------------|----------------------|
| **Format** | Freeform markdown narrative | Structured YAML fields in each change |
| **Conflict detection** | Manual reading | Automatic cycle and overlap detection |
| **Ordering** | Manually maintained list | `openspec change graph` and `openspec change next` |
| **Merge conflicts** | Everyone edits the same file | Each change owns its own metadata |
| **Enforcement** | Advisory only | Errors for cycles/missing deps, warnings for overlaps |

### Migration Path

`IMPLEMENTATION_ORDER.md` is **not removed or invalidated**. It remains useful as:

- **Narrative context** for humans explaining *why* an ordering exists
...
2. **Verify the graph matches the narrative:**
   ```bash
   openspec change graph
   ```
3. **Optionally, add a note to `IMPLEMENTATION_ORDER.md`:**
   ```markdown
   > **Note:** Dependency ordering is now tracked via stack metadata in each
   > change's `.openspec.yaml`. This file remains as narrative context. For
   > the current dependency graph, run `openspec change graph`.
   ```
4. **Stop manually updating `IMPLEMENTATION_ORDER.md`** for new changes — use `dependsOn` metadata instead.

### Key Point

Machine-readable stack metadata is the normative dependency source of truth. `IMPLEMENTATION_ORDER.md` is an optional narrative supplement, not a dependency source. Do not maintain ordering in two places — let the metadata drive ordering and keep the narrative file for human context only.
