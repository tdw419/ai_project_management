...
When multiple changes are in progress at the same time, they can interact in ways that are hard to track manually. Change stacking provides lightweight planning metadata so teams can express sequencing, dependencies, and overlap awareness between parallel changes.

### Why Stacking Matters

Three recurring problems in parallel development:

1. **No sequencing signal.** Teams cannot tell which change should land first.
2. **Large changes resist splitting.** A big proposal is hard to decompose into safe, independently mergeable slices.
3. **Hidden conflicts.** Parallel work can accidentally reintroduce assumptions already removed by another change.

Stack metadata solves these by making relationships between changes explicit and machine-readable.

### Stack Metadata Fields

Each change can include an optional `.openspec.yaml` file with stack metadata:

```yaml
# .openspec.yaml — inside the change directory
schema: spec-driven

# Changes that must land before this one
dependsOn:
  - add-auth-framework

# Capability markers this change exposes
provides:
  - auth-session-management

# Capability markers this change needs (advisory)
requires:
  - auth-session-management

# Spec/capability areas likely affected (advisory, overlap signal)
touches:
  - cli-init
  - cli-update

# Optional: parent change for split work
parent: refactor-api-layer
```

| Field | Type | Required | Purpose |
|-------|------|----------|---------|
| `dependsOn` | `string[]` | No | Changes that must be archived before this one. Source of truth for ordering. |
| `provides` | `string[]` | No | Capability markers this change makes available. Used for planning visibility. |
| `requires` | `string[]` | No | Capability markers this change needs. Advisory — does **not** create implicit dependency edges. |
| `touches` | `string[]` | No | Spec or capability areas likely affected. Triggers overlap warnings when multiple active changes share areas. |
| `parent` | `string` | No | References the parent change when this is a child slice from a split operation. |

**Key semantics:**

- `dependsOn` is the normative source of truth for execution and archive ordering.
- `provides`/`requires` are capability contracts for validation and planning visibility. They do **not** create implicit dependency edges — authors must still declare required ordering via `dependsOn`.
- `touches` is advisory only. Overlap warnings are informational, not blocking.
- All fields are optional and backward compatible. Changes without metadata work exactly as before.

### Stack-Aware Validation

When stack metadata is present, validation checks for planning issues:

| Check | Level | What It Detects |
|-------|-------|-----------------|
| Dependency cycles | Error | Circular `dependsOn` chains that prevent any ordering |
| Missing dependencies | Error | `dependsOn` references to change IDs that don't exist in active changes |
| Transitive blocking | Error | A change blocked by an unresolvable dependency chain |
| Overlap warnings | Warning | Multiple active changes that `touch` the same area |
| Unmatched `requires` | Warning | A `requires` marker with no matching `provides` in active changes |

Errors block progress (cycles, missing deps). Warnings are advisory (overlaps, unmatched markers).

### Parent/Child Slices

Large changes can be split into independently implementable child slices. Each child references its parent and declares a dependency:

```yaml
# In add-auth-2fa-slice-1/.openspec.yaml
schema: spec-driven
parent: add-auth-2fa
dependsOn:
  - add-auth-2fa
...
The parent change becomes a planning container — it tracks the overall goal while children carry the actual implementation tasks.

### Workflow Summary

```
1. Author adds stack metadata to .openspec.yaml (optional)
2. Validation detects cycles, missing deps, overlaps
3. `openspec change graph` shows topological order
4. `openspec change next` suggests unblocked changes
5. `openspec change split <id>` decomposes large changes
6. Archive proceeds in dependency order
```

For command details and usage examples, see [CLI Reference](cli.md).
...
| **Stack metadata** | Optional dependency and capability fields in a change's `.openspec.yaml` for sequencing parallel work |
| **Child slice** | A change created by splitting a parent, with `parent` and `dependsOn` metadata linking it back |
