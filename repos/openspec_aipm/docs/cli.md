...
| **Planning** | `change graph`, `change next`, `change split` | Dependency ordering and change decomposition |
...
Display the dependency graph for all active changes. Shows changes in topological order with depth-based indentation.
...
```
add-auth-framework
  add-auth-2fa <- add-auth-framework
add-dark-mode
    add-auth-sso <- add-auth-2fa
```

Changes at the same depth are sorted lexicographically by ID. Indentation reflects dependency depth — changes with no `dependsOn` appear at the left edge.

If a cycle is detected, the command fails with a deterministic error showing the cycle path:

```
Error: Dependency cycle detected: change-a -> change-b -> change-a
```
...
{
  "nodes": [
    { "id": "add-auth-framework", "depth": 0, "dependsOn": [] },
    { "id": "add-auth-2fa", "depth": 1, "dependsOn": ["add-auth-framework"] }
  ],
  "cycleError": null
}
```
...
Suggest unblocked changes in recommended implementation order. Only shows changes whose `dependsOn` targets are all present in the active set.
...
```bash
# Show which changes are ready to work on
openspec change next
...
add-auth-framework
add-dark-mode
add-auth-2fa (after: add-auth-framework)
```

Changes blocked by missing dependencies or cycles are excluded from the list.
...
```json
{
  "nodes": [
    { "id": "add-auth-framework", "depth": 0, "dependsOn": [] },
    { "id": "add-dark-mode", "depth": 0, "dependsOn": [] },
    { "id": "add-auth-2fa", "depth": 1, "dependsOn": ["add-auth-framework"] }
  ],
  "cycleError": null
}
...
Split a large change into child slices. Creates child change directories with `parent` and `dependsOn` metadata, generates stub `proposal.md` and `tasks.md` files, and converts the source change into a parent planning container.
...
| Argument | Required | Description |
|----------|----------|-------------|
| `change-id` | Yes | The active change to split |

**Options:**

| Option | Description |
|--------|-------------|
| `--slices <n>` | Number of child slices to create (default: 2) |
| `--names <names>` | Comma-separated names for child slices (overrides `--slices`) |
...
```bash
# Split into 2 default slices
openspec change split refactor-api-layer

# Split into 3 slices
openspec change split refactor-api-layer --slices 3

# Split with explicit names
openspec change split refactor-api-layer --names auth,payments,notifications

# Re-split after adjusting scope
openspec change split refactor-api-layer --overwrite

# JSON output
openspec change split refactor-api-layer --json
```
...
```
openspec/changes/
├── refactor-api-layer/               # Parent (converted to planning container)
│   ├── proposal.md                   # Updated with Split Status section
│   └── .openspec.yaml
├── refactor-api-layer-auth/          # Child slice 1
│   ├── proposal.md                   # Stub
│   ├── tasks.md                      # Stub
│   └── .openspec.yaml               # parent: refactor-api-layer, dependsOn: [refactor-api-layer]
└── refactor-api-layer-payments/      # Child slice 2
    ├── proposal.md
    ├── tasks.md
    └── .openspec.yaml
```

**Re-splitting.** Running `split` on an already-split change fails with a deterministic error listing existing children. Use `--overwrite` (or `--force`) to clean up existing children and regenerate:

```
Error: Change "refactor-api-layer" has already been split into:
  refactor-api-layer-auth, refactor-api-layer-payments.
Use --overwrite to re-split.
```
...
```json
{
  "parentId": "refactor-api-layer",
  "parentConverted": true,
  "children": [
    {
      "id": "refactor-api-layer-auth",
      "parent": "refactor-api-layer",
      "dependsOn": ["refactor-api-layer"]
    },
    {
      "id": "refactor-api-layer-payments",
      "parent": "refactor-api-layer",
      "dependsOn": ["refactor-api-layer"]
    }
  ]
