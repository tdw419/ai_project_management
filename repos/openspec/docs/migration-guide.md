...
## Migrating to Install Scope

OpenSpec v0.6+ introduces `installScope`, a global config setting that controls where tool artifacts (skills and commands) are written. This section explains how the transition works for existing users.

### What changed

Previously, all OpenSpec artifacts were written to project-local directories (e.g., `.claude/skills/`, `.cursor/commands/`). The only exception was Codex, which already installed commands to a global user directory (`$CODEX_HOME/prompts/` or `~/.codex/prompts/`).

With `installScope`, you can now choose a consistent strategy:

- **`project`** — all artifacts in the project directory (the old behavior)
- **`global`** — artifacts in user-level directories where the tool supports it, with automatic fallback to project-local for tools that don't

### How the migration works

OpenSpec uses **schema evolution** to handle the transition without breaking existing setups:

1. **New installations** (no config file exists) get `installScope: global` as the default.
2. **Existing installations** (config file exists but has no `installScope` key) effectively behave as `project` — your existing workflow is unchanged.
3. The `openspec config list` command annotates the source so you can see which case applies:

```
Install scope:
  installScope: project (legacy-default)
```

| Scenario | Config file state | Effective scope | Annotation |
|----------|------------------|-----------------|------------|
| Fresh install | No file | `global` | (new-default) |
| Existing user, no key | File exists, no `installScope` | `project` | (legacy-default) |
| Explicitly set | File has `installScope` | Whatever you set | (explicit) |

### Opting in to global scope

If you're an existing user and want to switch to global installs:
...
# Should show: installScope: global (explicit)

# 3. Regenerate artifacts for each project
openspec update

# 4. Optional: commit or clean up old project-local artifacts
# Project-local files that were replaced by global ones can be removed.
```

### Opting in to project scope (no-op for legacy users)

If you want to be explicit about project scope even though it's already your effective default:

```bash
openspec config set installScope project
```

This writes `"installScope": "project"` to your config file, changing the annotation from `(legacy-default)` to `(explicit)`. The behavior is identical — it just makes your intent explicit.

### No action required

If you're happy with your current setup, you don't need to do anything. Your existing projects continue to work exactly as before. The `installScope` field is only consumed during `openspec init` and `openspec update`, so in-flight changes are unaffected.
