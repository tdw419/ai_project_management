...
When multiple people (or agents) work on the same codebase, changes often share capabilities or affect the same spec areas. Change stacking provides lightweight planning metadata so teams can sequence, validate, and decompose work safely.

### Why Stacking Matters

Without explicit dependency information between changes:
...
Stack metadata lives in each change's `.openspec.yaml` file. All fields are optional and backward compatible — existing changes without stack metadata continue to work unchanged.

```yaml
# .openspec.yaml within a change folder
schema: spec-driven
created: 2025-06-15

# --- Stack metadata (optional) ---

# Names of other changes that must land before this one
dependsOn:
  - add-user-model

# Capability markers this change exposes to dependents
provides:
  - user-auth-endpoint

# Capability markers this change needs from its environment
requires:
  - user-persistence

# File paths or modules this change touches (advisory, for conflict detection)
touches:
  - src/auth/
  - src/middleware/

# Parent change name — marks this change as a slice of a larger change
parent: implement-oauth
```
...
|-------|---------|------------|
| `dependsOn` | Hard ordering: changes that must be archived first | ERROR if target missing or cycle detected |
| `provides` | Capability contract: what this change makes available | Advisory (used by `requires` matching) |
| `requires` | Capability precondition: what this change needs | WARNING if no active change provides it |
| `touches` | Overlap signal: areas this change likely affects | WARNING on overlap with other active changes |
| `parent` | Decomposition: marks this as a child slice | Links back to parent change |

**Key rule:** `requires` does not create an implicit dependency edge. If ordering is needed, authors must explicitly declare it via `dependsOn`. This keeps the dependency graph intentional and avoids surprising transitive ordering.

### Stack-Aware Validation

OpenSpec validates stack metadata across all active changes in four phases:

1. **Cycle detection** — dependency cycles are reported as errors (e.g., `a → b → a`)
2. **Missing dependency detection** — references to nonexistent changes are errors
3. **Transitive blocking** — changes blocked by errored or missing dependencies are flagged
4. **Advisory warnings** — overlap in `touches` and unmatched `requires` are surfaced as warnings

Only errors (cycles, missing deps, blocked changes) block progress. Warnings are informational.

### Viewing Dependency Order

The `openspec change graph` command displays a topological ordering of active changes:

```text
Dependency order:
  add-user-model
    add-auth-endpoints (depends on: add-user-model)
      add-oauth-flow (depends on: add-auth-endpoints)
  update-ci-config
```

Changes with no dependencies appear at the top. Changes that depend on others are indented beneath their dependencies.

### Finding What to Work On Next

The `openspec change next` command suggests unblocked changes:
...
When a change grows too large to land safely in one piece, `openspec change split` decomposes it into child slices:

```bash
# Split a large change into two child slices
openspec change split implement-oauth --slices oauth-core,oauth-ui
```

This creates:
- `openspec/changes/oauth-core/` with `.openspec.yaml` pointing to `parent: implement-oauth`
- `openspec/changes/oauth-ui/` with `.openspec.yaml` pointing to `parent: implement-oauth`
- The original `implement-oauth` change is marked as a parent planning container

Child slices can declare their own `dependsOn` to sequence within the parent. To re-split with different slices, pass `--overwrite` (or `--force`):

```bash
openspec change split implement-oauth --slices oauth-core,oauth-ui,oauth-tests --overwrite
```

### When to Split vs. Keep One Change

Split when:
- The change spans multiple independently deployable concerns
- Different people (or agents) will own different parts
- You want to archive parts incrementally

Keep as one change when:
- Everything is tightly coupled and must land atomically
- The change is small enough to review and test as a unit
- Splitting would create more coordination overhead than it saves

### Breaking Large Changes into Independently Mergeable Slices

When a change is too large to safely merge in one shot, use this approach:

1. **Identify natural seams.** Look for boundaries where one set of tasks can ship value independently. Common seams: new module vs. integration, backend vs. frontend, core logic vs. configuration.

2. **Declare capabilities.** In each child slice's `.openspec.yaml`, use `provides` to state what it exposes and `requires` to state what it needs from other slices.

3. **Sequence with `dependsOn`.** Explicitly order slices so that foundational changes land before dependents. This is the only mechanism that enforces ordering — `provides`/`requires` are for validation and visibility only.

4. **Run `openspec change graph`** to verify the dependency DAG is acyclic and the ordering is what you expect.

5. **Archive from the bottom up.** Archive leaf changes (no dependents) first, working up the dependency tree. `openspec change next` tells you which changes are currently unblocked.

Example workflow:

```bash
# 1. Split the large change
openspec change split add-full-auth --slices auth-model,auth-api,auth-ui

# 2. Add capability contracts and ordering to each slice
# Edit .openspec.yaml in each child:
#   auth-model: provides: [user-persistence]
#   auth-api: dependsOn: [auth-model], provides: [auth-endpoint], requires: [user-persistence]
#   auth-ui: dependsOn: [auth-api], requires: [auth-endpoint]

# 3. Verify the DAG
openspec change graph

# 4. Work bottom-up
openspec change next        # → auth-model is ready
# ... implement and archive auth-model ...
openspec change next        # → auth-api is now unblocked
# ... implement and archive auth-api ...
openspec change next        # → auth-ui is now unblocked
```

### Migration from IMPLEMENTATION_ORDER.md

Teams that previously used `openspec/changes/IMPLEMENTATION_ORDER.md` to document merge order should migrate to per-change stack metadata:

**Migration steps:**

1. Add `dependsOn` to each change's `.openspec.yaml` based on the ordering described in `IMPLEMENTATION_ORDER.md`
2. Optionally add `provides` and `requires` for capability contracts
3. Verify with `openspec change graph` that the topological order matches your intent
4. Keep `IMPLEMENTATION_ORDER.md` as optional narrative context during the transition — it is no longer the dependency source of truth

**Important:** `IMPLEMENTATION_ORDER.md` is not parsed by tooling. The machine-readable dependency graph comes exclusively from per-change `dependsOn` metadata. The markdown file may remain as a human-readable supplement, but it should not contradict the metadata.
...
OpenSpec's install scope setting controls where tool artifacts (skills and commands) are written during `openspec init` and `openspec update`.

### project vs. global

| Scope | Behavior |
|-------|----------|
| `project` | All artifacts written inside the project directory. Each project manages its own skills and commands. |
| `global` | Artifacts written to user-level directories where possible. Tools that don't support global installs fall back to project-local paths. |

### Cross-project implications

Setting `installScope: global` affects all OpenSpec projects on your machine that share the same global config. Key considerations:

**Shared commands.** When commands are installed globally (e.g., Codex commands at `$CODEX_HOME/prompts/`), they are shared across every project. Updating one project's OpenSpec files overwrites the shared commands. This is fine when all projects use the same profile and workflow set, but can cause conflicts when they don't.

**Profile and workflow consistency.** If you work across multiple projects with different profile or workflow selections, global commands will reflect the last project that ran `openspec update`. Consider using `installScope: project` in multi-project setups where profiles differ.

**Skills are always project-local.** Currently, skills are written to the project's `.tool/skills/` directory regardless of scope setting. This means skills are always per-project and won't conflict across projects.

**Team collaboration.** When `installScope: project`, skills and commands are committed to version control alongside the project. Team members who clone the repo get the OpenSpec configuration automatically. With `installScope: global`, each team member must run `openspec update` locally after cloning.

### When to use each

- **Use `project`** when you work on multiple projects with different tool configurations, collaborate with a team, or want artifacts version-controlled.
- **Use `global`** when you primarily use one project, prefer a single user-level setup, or want to avoid per-project artifact directories.

See [Supported Tools](supported-tools.md) for the full scope behavior reference and effective-scope fallback details.
...
| **Install scope** | The `installScope` setting (`global` or `project`) controlling where tool artifacts are written |
| **Effective scope** | The resolved scope actually used for a given tool surface after applying scope-support fallback logic |
| **Stack metadata** | Optional fields (`dependsOn`, `provides`, `requires`, `touches`, `parent`) in a change's `.openspec.yaml` for sequencing and decomposition |
| **Child slice** | A change created by splitting a parent change; linked via `parent` metadata |
