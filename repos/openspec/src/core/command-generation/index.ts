 ...
 * Usage:
 * ```typescript
 * import { generateCommands, CommandAdapterRegistry, type CommandContent, type InstallContext } from './command-generation/index.js';
 *
 * const contents: CommandContent[] = [...];
 * const adapter = CommandAdapterRegistry.get('cursor');
 * if (adapter) {
 *   const context: InstallContext = { scope: 'project' };
 *   const commands = generateCommands(contents, adapter, context);
