...
import { getGlobalConfig, type Delivery, type Profile, type InstallScope } from './global-config.js';
...
} from './shared/scope-resolver.js';
import type { InstallContext } from './command-generation/types.js';
import { getSkillDirForWorkflow } from './templates/tool-profile-registry.js';
...
// Populated from manifest-derived tool profile registry
// (this local copy is used only for removeSkillDirs/removeCommandFiles cleanup)
...
export class InitCommand {
  ...
  private async generateSkillsAndCommands(
    ...
    skillsInvocableTools: typeof tools;
  }> {
    ...
    const skillsInvocableTools: typeof tools = [];
    ...
    // Resolve per-tool command surfaces
    const surfaceMap = resolveCommandSurfaces(
      tools.map((t) => t.value),
      (toolId) => CommandAdapterRegistry.has(toolId)
    );

    // Fail fast: in delivery=commands, reject tools with 'none' command surface
    if (delivery === 'commands') {
      const unsupported = tools.filter((t) => surfaceMap.get(t.value) === 'none');
      if (unsupported.length > 0) {
        const names = unsupported.map((t) => t.value).join(', ');
        throw new Error(
          `Incompatible tools for commands delivery: ${names}. ` +
          `These tools have no command surface (neither adapter nor skills-invocable). ` +
          `Change delivery to 'both' or 'skills', or remove these tools from selection.`
        );
      }
    }

    // Interactive compatibility note: when delivery=commands includes skills-invocable tools,
    // explain that skills will be retained as their command surface
    if (delivery === 'commands') {
      const skillsInvocable = tools.filter((t) => surfaceMap.get(t.value) === 'skills-invocable');
      if (skillsInvocable.length > 0) {
        const names = skillsInvocable.map((t) => t.name).join(', ');
        console.log(chalk.dim(
          `Note: Skills retained as command surface for: ${names} (delivery=commands, command surface: skills-invocable)`
        ));
      }
    }
    ...
    // Process each tool with per-tool capability-aware logic
    for (const tool of tools) {
      const spinner = ora(`Setting up ${tool.name}...`).start();
      const surface = surfaceMap.get(tool.value) ?? 'none';
      ...
      try {
        ...
        const toolShouldRemoveSkills = !toolShouldGenerateSkills;
        const toolShouldRemoveCommands = !toolShouldGenerateCommands;
        ...
        if (toolShouldGenerateSkills) {
          ...
          const renderedSkills = renderSkills(projectPath, engineTool, skillTemplates, OPENSPEC_VERSION);
          await writeArtifacts(renderedSkills, []);
        }
        if (toolShouldRemoveSkills) {
        ...
        if (toolShouldGenerateCommands) {
          ...
          if (adapter) {
            const engineTool: ArtifactToolDescriptor = { toolId: tool.value, name: tool.name, skillsDir: tool.skillsDir };
            const renderedCmds = renderCommands(projectPath, engineTool, commandContents);
            await writeArtifacts([], renderedCmds);
          } else if (surface === 'skills-invocable') {
            // skills-invocable tools don't use adapters; this is expected, not a skip
            skillsInvocableTools.push(tool);
          } else {
            commandsSkipped.push(tool.value);
          }
        }
        if (toolShouldRemoveCommands) {
  ...
  private displaySuccessMessage(
    ...
    // Show skills-invocable tools using skills as command surface (step 2.4)
    if (results.skillsInvocableTools.length > 0) {
      const names = results.skillsInvocableTools.map((t) => t.name).join(', ');
      console.log(chalk.dim(`Skills used as command surface for: ${names}`));
    }

    // Show skipped commands (only for truly unhandled tools, not skills-invocable)
