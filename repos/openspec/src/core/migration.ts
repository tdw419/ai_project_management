...
import { getWorkflowSkillDir } from './profile-sync-drift.js';
...
function scanInstalledWorkflowArtifacts(
  ...
  for (const tool of tools) {
    ...
    for (const workflowId of ALL_WORKFLOWS) {
      const skillDirName = getWorkflowSkillDir(workflowId);
...
export function migrateIfNeeded(projectPath: string, tools: AIToolOption[]): void {
  ...
  // Legacy users default to 'project' scope since their existing installs are project-local
  if (rawConfig.installScope === undefined) {
    config.installScope = 'project';
  }
  saveGlobalConfig(config);

  console.log(`Migrated: custom profile with ${installedWorkflows.length} workflows`);
  console.log('Install scope set to "project" (your existing project-local installs).');
  console.log("Run 'openspec config profile' to switch to 'global' scope if preferred.");
