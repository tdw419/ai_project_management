...
import type { Profile, Delivery, InstallScope } from '../core/global-config.js';
import {
  getNestedValue,
  setNestedValue,
  deleteNestedValue,
...
type ProfileAction = 'both' | 'delivery' | 'workflows' | 'scope' | 'keep';
...
export function resolveCurrentProfileState(config: GlobalConfig): ProfileState {
  ...
  const installScope = config.installScope || 'project';
  const workflows = [
    ...getProfileWorkflows(profile, config.workflows ? [...config.workflows] : undefined),
  ];
  return { profile, delivery, workflows, installScope };
}
...
export function diffProfileState(before: ProfileState, after: ProfileState): ProfileStateDiff {
  ...
  if (before.installScope !== after.installScope) {
    lines.push(`installScope: ${before.installScope} -> ${after.installScope}`);
  }
  ...
  configCmd
    ...
    .action((options: { json?: boolean }) => {
      ...
      if (options.json) {
        ...
        // Annotate install scope
        const scopeValue = config.installScope;
        let scopeSource: string;
        if (rawConfig.installScope !== undefined) {
          scopeSource = '(explicit)';
        } else if (Object.keys(rawConfig).length === 0) {
          // No file on disk — new default
          scopeSource = '(new-default)';
        } else {
          // File exists but has no installScope — legacy default
          scopeSource = '(legacy-default)';
        }
        console.log(`\nInstall scope:`);
        console.log(`  installScope: ${scopeValue ?? 'project'} ${scopeSource}`);
      }
    });
  ...
  configCmd
    ...
    .action(async (preset?: string) => {
      ...
      if (preset === 'core') {
        ...
        // Preserve delivery and installScope settings
      ...
      try {
        ...
        console.log(`  Install scope: ${currentState.installScope}`);
        ...
        console.log(chalk.dim('  Install scope = global (user-level) or project (per-project) artifact locations'));
        console.log();
        ...
        const nextState: ProfileState = {
          ...
          installScope: currentState.installScope,
        };
        ...
        if (action === 'scope') {
          const scopeChoices: { value: InstallScope; name: string; description: string }[] = [
            {
              value: 'global',
              name: 'Global',
              description: 'Install to user-level directories (~/.config, XDG data dirs)',
            },
            {
              value: 'project',
              name: 'Project',
              description: 'Install to project-local directories (.claude/skills, etc.)',
            },
          ];
          for (const choice of scopeChoices) {
            if (choice.value === currentState.installScope) {
              choice.name += ' [current]';
            }
          }

          nextState.installScope = await select<InstallScope>({
            message: 'Install scope (where artifacts are written):',
            choices: scopeChoices,
            default: currentState.installScope,
          });
        }
        ...
        config.installScope = nextState.installScope;
        saveGlobalConfig(config);
