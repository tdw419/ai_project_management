...
changeCmd
  .command('graph')
  .description('Display dependency order for active changes')
  .option('--json', 'Output as JSON')
  .action(async (options?: { json?: boolean }) => {
    try {
      const changeCommand = new ChangeCommand();
      await changeCommand.graph(options);
    } catch (error) {
      console.error(`Error: ${(error as Error).message}`);
      process.exitCode = 1;
    }
...
changeCmd
  .command('next')
  .description('Suggest unblocked changes in recommended order')
  .option('--json', 'Output as JSON')
  .action(async (options?: { json?: boolean }) => {
    try {
      const changeCommand = new ChangeCommand();
      await changeCommand.next(options);
    } catch (error) {
      console.error(`Error: ${(error as Error).message}`);
      process.exitCode = 1;
    }
...
changeCmd
  ...
  .description('Split a change into child slices')
  .option('--slices <ids>', 'Comma-separated child slice IDs')
  .option('--overwrite', 'Allow re-splitting an already-split change')
  .option('--force', 'Alias for --overwrite')
  .action(async (changeId: string, options?: { slices?: string; overwrite?: boolean; force?: boolean }) => {
    try {
      const { splitChange } = await import('../core/split.js');
      if (!options?.slices) {
        throw new Error('--slices is required (comma-separated child IDs, e.g. --slices part-a,part-b)');
      }
      const sliceIds = options.slices.split(',').map((s: string) => s.trim()).filter(Boolean);
      const slices = sliceIds.map((id: string) => ({ id }));
      const result = await splitChange(process.cwd(), changeId, {
        slices,
        overwrite: options.overwrite || options.force || false,
      });
      console.log(`Split "${changeId}" into ${result.childIds.length} slices:`);
