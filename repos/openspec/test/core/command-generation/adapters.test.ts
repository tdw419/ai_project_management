...
import type { CommandContent, InstallContext } from '../../../src/core/command-generation/types.js';
...
describe('command-generation/adapters', () => {
  ...
  describe('codexAdapter', () => {
    ...
    it('should return an absolute path by default (backward compat = global)', () => {
    ...
    it('should return absolute path for global context', () => {
      const context: InstallContext = { scope: 'global' };
      const filePath = codexAdapter.getFilePath('explore', context);
      expect(path.isAbsolute(filePath)).toBe(true);
    });

    it('should return relative path for project context', () => {
    ...
    it('should produce consistent project paths for multiple commands', () => {
      const context: InstallContext = { scope: 'project' };
      expect(codexAdapter.getFilePath('explore', context)).toBe(path.join('.codex', 'prompts', 'opsx-explore.md'));
      expect(codexAdapter.getFilePath('apply', context)).toBe(path.join('.codex', 'prompts', 'opsx-apply.md'));
      expect(codexAdapter.getFilePath('new', context)).toBe(path.join('.codex', 'prompts', 'opsx-new.md'));
    });

    it('should produce consistent global paths for multiple commands', () => {
      const original = process.env.CODEX_HOME;
      delete process.env.CODEX_HOME;
      const context: InstallContext = { scope: 'global' };
      try {
        const base = path.join(os.homedir(), '.codex', 'prompts');
        expect(codexAdapter.getFilePath('explore', context)).toBe(path.join(base, 'opsx-explore.md'));
        expect(codexAdapter.getFilePath('apply', context)).toBe(path.join(base, 'opsx-apply.md'));
      } finally {
...
/opsx:apply to implement`,
  ...
  describe('scoped path behavior', () => {
    it('project-scoped adapters should ignore context and return relative paths', () => {
      const projectContext: InstallContext = { scope: 'project' };
      const globalContext: InstallContext = { scope: 'global' };

      // These adapters only support project scope - context is accepted but path is always project-relative
      expect(claudeAdapter.getFilePath('explore', projectContext)).toBe(
        path.join('.claude', 'commands', 'opsx', 'explore.md')
      );
      expect(claudeAdapter.getFilePath('explore', globalContext)).toBe(
        path.join('.claude', 'commands', 'opsx', 'explore.md')
      );
      expect(cursorAdapter.getFilePath('explore', projectContext)).toBe(
        path.join('.cursor', 'commands', 'opsx-explore.md')
      );
      expect(windsurfAdapter.getFilePath('explore', projectContext)).toBe(
        path.join('.windsurf', 'workflows', 'opsx-explore.md')
      );
    });

    it('Codex adapter should switch paths based on scope', () => {
      const original = process.env.CODEX_HOME;
      delete process.env.CODEX_HOME;
      try {
        const projectContext: InstallContext = { scope: 'project' };
        ...
        const projectPath = codexAdapter.getFilePath('explore', projectContext);
        const globalPath = codexAdapter.getFilePath('explore', globalContext);

        // Project path is relative
        expect(projectPath).toBe(path.join('.codex', 'prompts', 'opsx-explore.md'));
        expect(path.isAbsolute(projectPath)).toBe(false);

        // Global path is absolute
        expect(path.isAbsolute(globalPath)).toBe(true);
        expect(globalPath).toContain('opsx-explore.md');
    ...
    it('all adapters should accept InstallContext without error', () => {
      const context: InstallContext = { scope: 'project' };
      const adapters = [
        amazonQAdapter, antigravityAdapter, auggieAdapter, claudeAdapter,
        clineAdapter, codexAdapter, codebuddyAdapter, continueAdapter,
        costrictAdapter, crushAdapter, cursorAdapter, factoryAdapter,
        geminiAdapter, githubCopilotAdapter, iflowAdapter, kilocodeAdapter,
        opencodeAdapter, piAdapter, qoderAdapter, qwenAdapter, roocodeAdapter,
        windsurfAdapter,
      ];

      for (const adapter of adapters) {
        const filePath = adapter.getFilePath('test', context);
