...
describe('global-config', () => {
  ...
  describe('getGlobalConfig', () => {
    it('should return defaults when config file does not exist', () => {
      ...
      expect(config).toEqual({ featureFlags: {}, profile: 'core', delivery: 'both', installScope: 'global' });
    });
    ...
    describe('schema evolution', () => {
      ...
      it('should resolve missing installScope in legacy config to undefined (effective project)', () => {
        process.env.XDG_CONFIG_HOME = tempDir;
        const configDir = path.join(tempDir, 'openspec');
        const configPath = path.join(configDir, 'config.json');

        // Legacy config predating installScope
        fs.mkdirSync(configDir, { recursive: true });
        fs.writeFileSync(configPath, JSON.stringify({
          featureFlags: { existingFlag: true }
        }));
        ...
        expect(config.installScope).toBeUndefined();
        // Other fields still get defaults
        expect(config.profile).toBe('core');
        expect(config.delivery).toBe('both');
      });

      it('should set installScope to global for newly created configs', () => {
        process.env.XDG_CONFIG_HOME = tempDir;

        // No config file on disk → new config behavior
        const config = getGlobalConfig();

        expect(config.installScope).toBe('global');
      });

      it('should preserve explicit installScope from config file', () => {
        process.env.XDG_CONFIG_HOME = tempDir;
        const configDir = path.join(tempDir, 'openspec');
        const configPath = path.join(configDir, 'config.json');
      ...
      it('should round-trip installScope correctly', () => {
        process.env.XDG_CONFIG_HOME = tempDir;

        saveGlobalConfig({ featureFlags: {}, installScope: 'global' });
        const loaded = getGlobalConfig();

        expect(loaded.installScope).toBe('global');
      });
    });
  });
