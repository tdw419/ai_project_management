...
describe('config-schema', () => {
  ...
  describe('installScope', () => {
    it('should accept global installScope', () => {
      const result = validateConfig({ featureFlags: {}, installScope: 'global' });
      expect(result.success).toBe(true);
    });

    it('should accept project installScope', () => {
      const result = validateConfig({ featureFlags: {}, installScope: 'project' });
      expect(result.success).toBe(true);
    });

    it('should reject invalid installScope values', () => {
      const result = validateConfig({ featureFlags: {}, installScope: 'local' });
      expect(result.success).toBe(false);
      expect(result.error).toContain('installScope');
    });

    it('should accept config without installScope', () => {
      const result = validateConfig({ featureFlags: {} });
      expect(result.success).toBe(true);
    });
  });

  describe('validateConfigKeyPath with installScope', () => {
    it('should allow installScope key', async () => {
      const { validateConfigKeyPath } = await import('../../src/core/config-schema.js');
      expect(validateConfigKeyPath('installScope').valid).toBe(true);
    });

    it('should reject nested keys under installScope', async () => {
      const { validateConfigKeyPath } = await import('../../src/core/config-schema.js');
      expect(validateConfigKeyPath('installScope.nested').valid).toBe(false);
    });
  });
});
