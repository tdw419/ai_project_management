...
describe('config key validation', () => {
  ...
  it('allows installScope key', async () => {
    const { validateConfigKeyPath } = await import('../../src/core/config-schema.js');
...
describe('config profile command', () => {
  ...
  it('config schema should accept valid installScope values', async () => {
    const { validateConfig } = await import('../../src/core/config-schema.js');

    expect(validateConfig({ featureFlags: {}, installScope: 'global' }).success).toBe(true);
    expect(validateConfig({ featureFlags: {}, installScope: 'project' }).success).toBe(true);
  });

  it('config schema should reject invalid installScope values', async () => {
    const { validateConfig } = await import('../../src/core/config-schema.js');

    const result = validateConfig({ featureFlags: {}, installScope: 'invalid' });
    expect(result.success).toBe(false);
  });
});
