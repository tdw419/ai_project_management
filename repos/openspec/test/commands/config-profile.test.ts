...
describe('diffProfileState workflow formatting', () => {
  it('uses explicit "removed" wording when workflows are deleted', async () => {
    ...
    const diff = diffProfileState(
      { profile: 'custom', delivery: 'both', workflows: ['propose', 'sync'], installScope: 'project' },
      { profile: 'custom', delivery: 'both', workflows: ['propose'], installScope: 'project' },
    );
  ...
  it('uses explicit labels when workflows are added and removed', async () => {
    ...
    const diff = diffProfileState(
      { profile: 'custom', delivery: 'both', workflows: ['propose', 'sync'], installScope: 'project' },
      { profile: 'custom', delivery: 'both', workflows: ['propose', 'verify'], installScope: 'project' },
    );
  ...
  it('detects installScope change', async () => {
    const { diffProfileState } = await import('../../src/commands/config.js');

    const diff = diffProfileState(
      { profile: 'core', delivery: 'both', workflows: ['propose', 'explore', 'apply', 'archive'], installScope: 'project' },
      { profile: 'core', delivery: 'both', workflows: ['propose', 'explore', 'apply', 'archive'], installScope: 'global' },
    );

    expect(diff.hasChanges).toBe(true);
    expect(diff.lines).toEqual(['installScope: project -> global']);
  });

  it('detects installScope and delivery change together', async () => {
    const { diffProfileState } = await import('../../src/commands/config.js');

    const diff = diffProfileState(
      { profile: 'core', delivery: 'both', workflows: ['propose', 'explore', 'apply', 'archive'], installScope: 'project' },
      { profile: 'core', delivery: 'skills', workflows: ['propose', 'explore', 'apply', 'archive'], installScope: 'global' },
    );

    expect(diff.hasChanges).toBe(true);
    expect(diff.lines).toContain('delivery: both -> skills');
    expect(diff.lines).toContain('installScope: project -> global');
  });
});
...
describe('config profile interactive flow', () => {
  ...
  it('core preset should preserve delivery and installScope settings', async () => {
    const { saveGlobalConfig, getGlobalConfig } = await import('../../src/core/global-config.js');
    const { select, checkbox, confirm } = await getPromptMocks();

    saveGlobalConfig({ featureFlags: {}, profile: 'custom', delivery: 'skills', workflows: ['explore'], installScope: 'global' });
  ...
  it('scope-only action should change installScope without affecting delivery or workflows', async () => {
    const { saveGlobalConfig, getGlobalConfig } = await import('../../src/core/global-config.js');
    const { select, checkbox } = await getPromptMocks();

    saveGlobalConfig({ featureFlags: {}, profile: 'core', delivery: 'both', workflows: ['propose', 'explore', 'apply', 'archive'], installScope: 'project' });
    select.mockResolvedValueOnce('scope');
    select.mockResolvedValueOnce('global');
  ...
  it('scope picker should mark current option inline', async () => {
    const { saveGlobalConfig } = await import('../../src/core/global-config.js');
    const { select } = await getPromptMocks();
    ...
    expect(select).toHaveBeenCalledTimes(2);
    const scopeCall = select.mock.calls[1][0];
    expect(scopeCall.message).toBe('Install scope (where artifacts are written):');
    expect(scopeCall.choices).toEqual(expect.arrayContaining([
      expect.objectContaining({ value: 'project', name: 'Project [current]' }),
      expect.objectContaining({ value: 'global', name: 'Global' }),
    ]));
  });

  it('action picker should include install scope option', async () => {
    const { saveGlobalConfig } = await import('../../src/core/global-config.js');
    const { select } = await getPromptMocks();
    ...
    expect(firstCall.choices).toEqual(expect.arrayContaining([
      expect.objectContaining({
        value: 'scope',
        name: 'Install scope',
        description: 'Switch between global (user-level) and project (per-project) installs',
      }),
    ]));
  });

  it('install scope header should appear in current profile settings', async () => {
    const { saveGlobalConfig } = await import('../../src/core/global-config.js');
    const { select } = await getPromptMocks();

    saveGlobalConfig({ featureFlags: {}, profile: 'core', delivery: 'both', workflows: ['propose', 'explore', 'apply', 'archive'], installScope: 'global' });
    select.mockResolvedValueOnce('keep');

    await runConfigCommand(['profile']);

    expect(consoleLogSpy).toHaveBeenCalledWith('  Install scope: global');
  });

  it('resolveCurrentProfileState should default installScope to project for legacy configs', async () => {
    const { resolveCurrentProfileState } = await import('../../src/commands/config.js');

    // Legacy config: installScope is undefined
    const state = resolveCurrentProfileState({ featureFlags: {}, profile: 'core', delivery: 'both' });

    expect(state.installScope).toBe('project');
  });
